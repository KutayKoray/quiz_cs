...
async function getQuestionById(soruId) {
    try {
        ...
        console.log('dogru_cevap:', dogru_cevap);
...
function finishExam() {
    ...
    console.log('Doğru cevap sayısı:', dogru_cevaplar);
    console.log('Yanlış cevap sayısı:', wrong_answers);
    console.log('Toplam soru sayısı:', total_questions);
    console.log('Skor:', score);

    update_user_stats(total_questions, dogru_cevaplar, wrong_answers, score, user);
}
...
function display_stats() {
    ...
    statsList.innerHTML = `
        <li>Correct Questions: ${stats.dogru_cevap}</li>
...
function displayQuestion(question) {
    ...
    questionContainer.innerHTML = `
        ...
        <h2>${counter}. Soru</h2>
        <br>
        <img src="./uploads/${question.imageFileName}" 
             alt="Soru resmi" 
...
// -kullanıcın işaretlediği cevaplar alınacak kaydedilecek sınav bittiğinde kontrol edilip doğru ve yanlış scor sayısı belirlenecek. Bunun için bir veri tipi oluşturulacak her sorunun id'si, sorunun dogru_cevap bilgisi ve kullanıcın işaretlediği cevaplar bu veri tipine kaydedilecek ve bu veri tipini tutacak bir liste oluşturulacak. En sonda bu liste ile doğru ve yanlış sayısı belirlenecek
...
function handle_answer(event) {
    ...
    switch (buttonId) {
        case 'optionA':
            saveAnswer(question_id, dogru_cevap, user_answer);
            break;
        case 'optionB':
            saveAnswer(question_id, dogru_cevap, user_answer);
            break;
...
function saveAnswer(question_id, dogru_cevap, user_answer) {
    user_answers.push({
...
function handle_user_answers() {
    ...
    getLatestAnswers(user_answers).forEach(answer => {
        if (answer.dogru_cevap === answer.user_answer) {
            dogru_cevaplar++;
            score += 3;
        } else {
            wrong_answers++;
    ...
    console.log('dogru_cevaplar:', dogru_cevaplar);
